...
static void
process_udp(void *ethd,void *iphd, void *data,uint16_t datalen,unsigned portid)
{
	struct ether_hdr *eth = (struct ether_hdr *)ethd;
	struct ipv4_hdr *iphdr = (struct ipv4_hdr *)iphd;
    ...
    /*swap port*/
    port = udphdr->src_port;
    udphdr->src_port = udphdr->dst_port;
    udphdr->dst_port = port;
  /*这里没有做checksum,是因为源地址和目地地址仅仅做了交换，并没有改变，
  *假如开发nat功能时，这一步一定要做，要不然协议栈会把数据包丢弃导致数据收发异常*/
}
	
static void
process_packets(struct rte_mbuf *m, unsigned portid)
{
	int sent;
	...
	datalen = rte_pktmbuf_data_len(m);
	if(datalen < 44) return;

	eth = rte_pktmbuf_mtod(m, struct ether_hdr *);
	data = (uint8_t *)eth;
	if(ETHER_TYPE_ARP == rte_be_to_cpu_16(eth->ether_type))
	{
			printf("arp package\n");
	}
	else if(ETHER_TYPE_IPv4 == rte_be_to_cpu_16(eth->ether_type))
	{   
		  
			iphdr = (struct ipv4_hdr *)(data + sizeof(struct ether_hdr));
			datalen -= sizeof(struct ether_hdr);
			
			data = (uint8_t *)iphdr + sizeof(struct ipv4_hdr);
			
			if(iphdr->next_proto_id == IPPROTO_UDP)
			{
					datalen -= sizeof(struct ipv4_hdr);
					process_udp(eth,iphdr,data,datalen,portid);
			}
	}
	else
	{
